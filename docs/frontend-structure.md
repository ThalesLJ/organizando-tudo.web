...
- `.ui-input` in `globals.css` overrides browser autofill (`:-webkit-autofill` and `:autofill`) so the filled field keeps the theme surface: stacked inset shadows use `var(--input-background)` over `var(--bg-primary)` so transparent inputs match the page and opaque custom input backgrounds still apply
