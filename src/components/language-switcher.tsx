...
export function LanguageSwitcher() {
  ...
  return (
    <div className="fixed right-4 top-4 z-50 rounded-lg border border-[var(--border-color)] bg-[var(--input-background)] shadow-2xl">
      <select
        suppressHydrationWarning
        value={locale}
        onChange={(event) => void handleChange(event.target.value as AppLocale)}
        className="h-9 rounded-lg border border-[var(--border-color)] bg-[var(--input-background)] px-3 text-xs font-medium text-[var(--text-primary)] outline-none"
      >
